    ...
    <div>
      ...
      {loading ? <Loader /> :
        <div className="details__container">
          <Link to={`/${type}s/${id - 1}`} className="details__container__arrow details__container__arrow--prev">
                  ...
                  <div className="details__section">
                    ...
                    <div className="details__section__desc">
                      <p>{card.name.split(' ')[0]} is originally from &nbsp;
                        {
                          card.origin.name !== 'unknown' ? (<Link to={`/locations/${extractId(card.origin.url)}`}> {card.origin.name} </Link>)
                            : (<span> an unknown place </span>)
                        }
                        and it's last known location is &nbsp;
                        {
                          card.location.name !== 'unknown' ? (<Link to={`/locations/${extractId(card.location.url)}`}> {card.location.name}. </Link>)
          ...
          <Link to={`/${type}s/${parseInt(id) + 1}`} className="details__container__arrow details__container__arrow--next">
